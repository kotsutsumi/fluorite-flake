...
- 変更したソースと対応する `docs/reviews/src/...` のレビュー文書を同じイテレーション内で更新し、古い内容を残さないこと
...
`docs/reviews/src` を含む既存のレビュー文書も必ず最新コードと照合し、差異があれば同じイテレーションの中で更新を完了させる事
...
作業開始時、重複作業されないように、`[ STATUS:IMPLEMENTING ]`に書き換える。
実装後、`docs/implement-reports` に同じファイル名で、実装報告を記載してください。
作業後、重複作業されないように、`[ STATUS:DONE ]`に書き換える。

...
export async function collectBlobConfiguration(
    ...
    if (blobMode === "existing") {
        ...
        try {
            ...
            try {
                console.log(
                    `🔑 選択されたストア用のトークンを自動生成中: ${existingStore.storeId}`
                );
                ...
                if (tokenResult.success) {
                    ...
                    console.log(
                        `✅ 新しいトークンを自動生成しました: ${tokenResult.tokenId}`
                    );
                }
            } catch (tokenError) {
                console.warn(
                    "⚠️ 新しいトークン生成に失敗しました。元のトークンを使用します:",
                    tokenError
                );
                // 既存トークンで続行
            }
    ...
    try {
        ...
        if (
            ...
            console.warn(
                "Blobストアの作成に必要な認証情報が不足しているため、Blob設定をスキップします。"
            );
...
function showTokenAcquisitionGuide(): void {
    ...
    console.log(
        "4. 作成されたBlobストアの Settings → Environment Variables から"
    );
...
async function promptForToken(
    options: { message?: string; showGuide?: boolean } = {}
): Promise<string | null> {
    const messages = getMessages();
    let attempts = 0;
    const maxAttempts = 3;
    ...
    while (attempts < maxAttempts) {
        const token = await text({
            ...
            validate: (value: string) => {
                ...
                if (
                    !(
                        trimmedToken.startsWith("vercel_blob_rw_") ||
                        trimmedToken.startsWith("blob_rw_")
                    )
                ) {
...
async function createNewStore(projectName: string): Promise<{
    ...
    if (requiresTokenForCreation) {
        const initialToken = await promptForToken({
            message:
                "BLOB_READ_WRITE_TOKEN を入力してください（新規作成に使用します）：",
            showGuide: true,
        });
    ...
    while (true) {
        try {
            ...
            if (
                ...
                console.warn(
                    "提供された認証情報で Blob ストアを作成できませんでした。再入力してください。"
                );
...
async function selectExistingStore(
    token: string
): Promise<{ storeId: string; storeName: string; storeUrl: string }> {

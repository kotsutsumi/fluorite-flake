...
export class AWSAdapter extends BaseServiceAdapter {
    ...
    async initialize(config?: ServiceConfig): Promise<void> {
        ...
        try {
            ...
            throw new Error(
                'AWS CLI is not installed. Please install it first: https://aws.amazon.com/cli/'
            );
        }
    }
    ...
    async connect(): Promise<void> {
        if (!(await this.isAuthenticated())) {
            throw new Error('Not authenticated. Please configure AWS credentials.');
        }
    ...
    async healthCheck(): Promise<HealthStatus> {
        ...
        try {
            ...
            checks.push({
                ...
                message: 'AWS CLI is not available',
                duration: Date.now() - startTime,
            });
        }
        ...
        if (isAuth) {
            try {
                ...
                checks.push({
                    name: 'Service Access',
                    status: 'pass' as const,
                    message: 'Can access AWS services',
                    duration: Date.now() - startTime,
                });
            } catch {
                checks.push({
                    ...
                    message: 'Limited service access',
                    duration: Date.now() - startTime,
                });
            }
        }
    ...
    async getDashboardData(_options?: DashboardDataOptions): Promise<ServiceDashboardData> {
        ...
        try {
            ...
            try {
                ...
                data.lambdaFunctions = result.Functions.map((fn: any) => ({
            ...
            try {
                ...
                data.s3Buckets = result.Buckets.map((bucket: any) => ({
                    name: bucket.Name,
            ...
            try {
                ...
                data.rdsDatabases = result.DBInstances.map((db: any) => ({

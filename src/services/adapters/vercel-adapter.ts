...
export class VercelAdapter extends BaseServiceAdapter {
            ...
            if (!type || type === 'deployment') {
                ...
                resources.push(
                    ...deployments.map((d) => ({
                        ...
                        type: 'deployment',
                        name: d.name,
                        status: d.state.toLowerCase() as any,
            ...
            if (!type || type === 'domain') {
                ...
                resources.push(
                    ...domains.map((d) => ({
                        ...
                        type: 'domain',
                        name: d.name,
                        status: d.verified ? 'active' : ('pending' as any),
                        createdAt: d.createdAt,
    ...
    private async deployProject(_projectId: string, _params?: any): Promise<ActionResult> {
        try {
            const cmd = this.vercelConfig.team

...
export class CloudflareAdapter extends BaseServiceAdapter {
    ...
    async getDashboardData(_options?: DashboardDataOptions): Promise<ServiceDashboardData> {
        try {
            ...
            const dashboardData: ServiceDashboardData = {
                ...
                summary: {
                    totalWorkers: (data.workers as WranglerWorker[])?.length || 0,
                    totalR2Buckets: (data.r2Buckets as WranglerR2Bucket[])?.length || 0,
                    totalKVNamespaces: (data.kvNamespaces as WranglerKVNamespace[])?.length || 0,
                },
            };
        ...
        try {
            ...
            return {
                ...
                performance: {
                    ...
                    throughput: 0,
                    errorRate: 0,
                    activeConnections: (data.workers as WranglerWorker[])?.length || 0,
                },
                usage: {
                    requests: 0,
                    dataTransfer: 0,
    ...
    async *getLogs(_options?: LogOptions): AsyncIterable<LogEntry> {
        ...
        try {
            ...
            for (const worker of (data.workers as WranglerWorker[]) || []) {
                yield {
            ...
            for (const bucket of (data.r2Buckets as WranglerR2Bucket[]) || []) {
                yield {
    ...
    async listResources(type?: string): Promise<Resource[]> {
        ...
        try {
            ...
            if (!type || type === 'worker') {
                for (const worker of (data.workers as WranglerWorker[]) || []) {
                    resources.push({
                        ...
                        updatedAt: Date.now(),
                        metadata: worker as unknown as Record<string, unknown>,
            ...
            if (!type || type === 'r2-bucket') {
                for (const bucket of (data.r2Buckets as WranglerR2Bucket[]) || []) {
                    resources.push({
                        ...
                        updatedAt: Date.now(),
                        metadata: bucket as unknown as Record<string, unknown>,
            ...
            if (!type || type === 'kv-namespace') {
                for (const kv of (data.kvNamespaces as WranglerKVNamespace[]) || []) {
                    resources.push({
                        id: kv.id,
                        type: 'kv-namespace',
                        name: kv.title,
                        ...
                        createdAt: Date.now() - 86400000,
                        updatedAt: Date.now(),
                        metadata: kv as unknown as Record<string, unknown>,

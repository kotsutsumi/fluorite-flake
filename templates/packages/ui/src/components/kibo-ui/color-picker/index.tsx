...
export const ColorPicker = ({
  ...
  useEffect(() => {
    if (value) {
      ...
      setHue(nextColor.hue() ?? 0);
      setSaturation(nextColor.saturationl() ?? 0);
      setLightness(nextColor.lightness() ?? 0);
      setAlpha((nextColor.alpha() ?? 1) * 100);
    }
  }, [value]);
...
export const ColorPickerEyeDropper = ({ className, ...props }: ColorPickerEyeDropperProps) => {
  ...
  const handleEyeDropper = async () => {
    try {
      ...
      const [rawHue, rawSaturation, rawLightness] = color.hsl().array();
      const safeHue = typeof rawHue === "number" ? rawHue : 0;
      const safeSaturation = typeof rawSaturation === "number" ? rawSaturation : 0;
      const safeLightness = typeof rawLightness === "number" ? rawLightness : 0;

      setHue(safeHue);
      setSaturation(safeSaturation);
      setLightness(safeLightness);
      setAlpha(100);
    } catch (_error) {}
  };

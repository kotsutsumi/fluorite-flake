...
>({
  ...
  const handleDragOver = (event: DragOverEvent) => {
    ...
    if (overColumn && activeColumn !== overColumn) {
      ...
      const safeOverIndex = overIndex === -1 ? activeIndex : overIndex;
      const overCard = newData[safeOverIndex];

      if (!overCard) {
        return;
      }

      newData[activeIndex] = { ...activeCard, column: overColumn };
      newData = arrayMove(newData, activeIndex, safeOverIndex);

...
type FeatureWithPosition = GanttFeature & { subRow: number };
...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
  const timelineStartEntry = context.timelineData[0];

  if (!timelineStartEntry) {
    return new Date();
  }

  const timelineStartDate = new Date(timelineStartEntry.year, 0, 1);
...
export const GanttFeatureRow: FC<GanttFeatureRowProps> = ({
  ...
  for (const feature of sortedFeatures) {
    ...
    while (subRow < subRowEndTimes.length) {
      const endTime = subRowEndTimes[subRow];
      if (!endTime || endTime <= feature.startAt) {
        break;
      }
      subRow++;
    }
  ...
  return (
    ...
    >
      {featureWithPositions.map(({ subRow, ...feature }) => (
        <div
          className="absolute w-full"
          key={feature.id}
          style={{
            top: `${subRow * subRowHeight}px`,
...
export const GanttProvider: FC<GanttProviderProps> = ({
  ...
  const scrollToFeature = useCallback(
    (feature: GanttFeature) => {
      ...
      const timelineStartEntry = timelineData[0];
      if (!timelineStartEntry) {
        return;
      }

...
export function AccessHistoryClient({ user: _user, initialTab }: AccessHistoryContentProps) {
  ...
  const [startDate, setStartDate] = useState<Date>(
    subDays(new Date(), DEFAULT_ACCESS_HISTORY_LOOKBACK_DAYS)
  );
  ...
  const calculateTotalAccesses = useCallback(
    (totalCount: number | null, logsLength: number): number => {
      if (totalCount !== null) {
        return totalCount;
      }
      if (logsLength === 0) {
                      ...
                      <PieChart>
                        ...
                        >
                          {stats.topPlatforms.map((entry) => (
                            <Cell
                              fill={
                                platformColors[entry.platform as keyof typeof platformColors] ||
                  ...
                  {stats?.recentActivity.map((log) => (
                    <div className="flex items-center space-x-4" key={log.id}>
                      <div className="flex items-center space-x-2">
                        ...
                        <Badge variant="outline">{log.platform?.toUpperCase() || "UNKNOWN"}</Badge>
                      </div>

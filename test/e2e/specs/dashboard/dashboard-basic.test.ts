...
describe("ダッシュボード基本機能 E2E テスト", () => {
    ...
    describe("ダッシュボード起動", () => {
        test("ダッシュボードが正常に起動する", async () => {
            ...
            await waitForOutput(
                dashboardProcess,
                "Vercel",
                PERFORMANCE_THRESHOLDS.DASHBOARD.STARTUP
            );
    ...
    describe("キーボード操作", () => {
        ...
        test("サービス切り替えキーが正常に動作する", async () => {
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.SERVICES.turso
            );
            await waitForOutput(
                dashboardProcess!,
                "Turso",
                PERFORMANCE_THRESHOLDS.DASHBOARD.SERVICE_SWITCH
            );

            // Supabase に切り替え
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.SERVICES.supabase
            );
            await waitForOutput(
                dashboardProcess!,
                "Supabase",
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.SERVICES.github
            );
            await waitForOutput(
                dashboardProcess!,
                "GitHub",
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.SERVICES.vercel
            );
            await waitForOutput(
                dashboardProcess!,
                "Vercel",
        ...
        test("タブ切り替えキーが正常に動作する", async () => {
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.TABS.operations
            );
            await waitForOutput(
                dashboardProcess!,
                "Operations",
                PERFORMANCE_THRESHOLDS.DASHBOARD.TAB_SWITCH
            );

            // Logs タブに切り替え
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.TABS.logs
            );
            await waitForOutput(
                dashboardProcess!,
                "Logs",
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.TABS.metrics
            );
            await waitForOutput(
                dashboardProcess!,
                "Metrics",
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.TABS.overview
            );
            await waitForOutput(
                dashboardProcess!,
                "Overview",
        ...
        test("終了キーでダッシュボードが正常に終了する", async () => {
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.ACTIONS.quit
            );
        ...
        test("ESCキーでダッシュボードが正常に終了する", async () => {
            ...
            sendKey(
                dashboardProcess!,
                DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.ACTIONS.escape
            );
    ...
    describe("UI表示", () => {
        ...
        test("サイドバーに全サービスが表示される", async () => {
            for (const service of DASHBOARD_CONFIG.SERVICES) {
                // 各サービスに切り替えて表示を確認
                const shortcut =
                    DASHBOARD_CONFIG.KEYBOARD_SHORTCUTS.SERVICES[service];
    ...
    describe("パフォーマンステスト", () => {
        ...
        test("サービス切り替え速度が基準内", async () => {
            ...
            expect(averageTime).toBeLessThanOrEqual(
                PERFORMANCE_THRESHOLDS.DASHBOARD.SERVICE_SWITCH
            );
        });
    });
